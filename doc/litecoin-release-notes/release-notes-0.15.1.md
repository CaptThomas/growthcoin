Growthcoin Core version *0.15.1* is now available from:

  <https://download.growthcoin.org/growthcoin-0.15.1/>
...
A number of changes to the way Growthcoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 growths per kB (0.00001 GCO). This is to prepare for dropping the minimum transaction fee to 0.1 growths per kB (0.0001 GCO) in the next release.
