...
#if defined(NDEBUG)
# error "Growthcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Growthcoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        for (CTxMemPool::txiter it : allConflicting)
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s GCO additional fees, %d delta bytes\n",
...
{
    double halvings = nHeight / (double) consensusParams.nSubsidyHalvingInterval;
   ...
   // nSubsidy >>= halvings;
   nSubsidy = nSubsidy * pow (0.5, halvings);
    return nSubsidy;
}
...
void ThreadScriptCheck() {
    RenameThread("growthcoin-scriptch");
